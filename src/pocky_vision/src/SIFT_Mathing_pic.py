...
import logging
from pocky_vision.srv import pocky_data
from pocky_vision.msg import ROI_array_bottom,ROI_bottom
from pocky_vision.msg import ROI_array_top,ROI_top
from pocky_vision.msg import vision_state
            ...
            if(X_side>=50 and X_side<=2000) and (Y_side>=50 and Y_side<=2000):
                if ((X_side_Less == 0 and Y_side_Less == 0) or (X_side_Less <= 6 and Y_side_Less <= 6)):
                    if ((X_center_point_Less == 0 and Y_center_point_Less == 0) or (X_center_point_Less <= 5 and Y_center_point_Less <= 5)):
                        ...
                        CenterY.append(Y_abs)
                            # if 'W' not in box:
                            #     if box_7 in Match_image:
                            #         bottom_box.append('W')
                            # if 'R' not in box:
                            #     if box_8 in Match_image:
                            #         bottom_box.append('R')
                            # if 'Y' not in box:
                            #     if box_9 in Match_image:
                            #         bottom_box.append('Y')
                            # if 'G' not in box:
                            #     if box_10 in Match_image:
                            #         bottom_box.append('G')
                            # if 'WP' not in box:
                            #     if box_11 in Match_image:
                            #         bottom_box.append('WP')
                            # if 'GP' not in box:
                            #     if box_12 in Match_image:
    ...
    try:
        while True:
            ...
            while(z < x1-20):
                ...
                if(cm < 0.535 and cm > 0.52): #cm < 0.535 and cm > 0.52 手臂原點高度
...
def RealSense3(topA,topB):
    try:
        while True:
            ...
            while(z < x1-20):
                cm = depth_frame_1.get_distance(topA[z],topB[z])
                if(cm < 0.56 and cm > 0.54): #cm < 0.56 and cm > 0.54 手臂原點高度
                    # print("depth value in m:{0}",cm)
                    color_image_1 = cv.circle(color_image_1,(topA[z],topB[z]),10,(0,255,255),-1)
                z = z+1
...
def contours_demo(image,x,y):
    ...
    __,contours,hierarchy = cv.findContours(edge_output,cv.RETR_TREE,cv.CHAIN_APPROX_SIMPLE)
...
if __name__ == '__main__':
    ...
    #pocky_srv = rospy.Service('',pocky_data_sent)
        ...
        while(1):            
            if start_input == 1:
                ...
                if(len(top_box)==6):
                    ...
                    ROI_TOP_ARRAY = ROI_array_top()
                    ROI_BOTTOM_ARRAY = ROI_array_bottom()
                    


                    ROI_TOP.top_box = top_box
                    ROI_TOP.top_CenterX = top_CenterX
                    ROI_TOP.top_CenterY = top_CenterY
                    ROI_TOP.top_Angle = top_Angle

                    TOP_list.append(ROI_TOP)
                    ROI_TOP_ARRAY.ROI_top_list = TOP_list

                    pub_top.publish(ROI_TOP_ARRAY)

                    ROI_BOTTOM.bottom_box = bottom_box
                    ROI_BOTTOM.bottom_CenterX = bottom_CenterX
                    ROI_BOTTOM.bottom_CenterY = bottom_CenterY
                    ROI_BOTTOM.bottom_Angle = bottom_Angle

                    BOTTOM_list.append(ROI_BOTTOM)
                    ROI_BOTTOM_ARRAY.ROI_bottom_list = BOTTOM_list

                    pub_bottom.publish(ROI_BOTTOM_ARRAY)

                    vision_state_flag = vision_state()
                    vision_state_flag.state = True
                    pub_visionstae.publish(vision_state_flag)
